 ...
 */
public struct TinyVideoPlayerDefaults: Sendable {
...
public final class TinyVideoPlayer: NSObject, TinyVideoPlayerProtocol, TinyLogging, @unchecked Sendable {
    ...
    public let loggingLevel: TinyLoggingLevel = .info
    ...
    private var externalPlaybackObservation: NSKeyValueObservation? = nil
    ...
    public var hidden: Bool = false {
        ...
        didSet {
            DispatchQueue.main.async {
                if self.hidden {
                    for (_, view) in self.projectionViewStore {
                        view.isHidden = true
                    }
    ...
    public override init() {
        ...
        self.externalPlaybackObservation = self.observe(\.player.isExternalPlaybackActive) { object, change in
            NotificationCenter.default.post(
                name: .ExternalPlaybackStateChanged,
                object: nil,
                userInfo: [
    ...
    @MainActor fileprivate func setupPlayerWithLoadedAsset(asset: AVAsset) {
        ...
        if context == &TinyVideoPlayerAVPlayerObservationContext {
            ...
            if #available(iOS 10.0, tvOS 10.0, *) {
                ...
                if keyPath == #keyPath(AVPlayer.timeControlStatus) {
                    ...
                    @unknown default:
                        infoLog("New unknown state at the time of coding was added.")
                        updatePlaybackState(.unknown)
                    }
                }
            }
...
extension TinyVideoPlayer {
    ...
    public func generateVideoProjectionView() -> TinyVideoProjectionView {
        var projectionView: TinyVideoProjectionView? = nil

        DispatchQueue.main.sync {
            projectionView = TinyVideoProjectionView()
...
extension NSNotification.Name {
    static let ExternalPlaybackStateChanged = Notification.Name("external-playback-changed")
}
